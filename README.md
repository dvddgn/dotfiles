...
| Script | What it does |
|---|---|
| `wt.sh` | `wt new/done/rename/rm/ls` — creates a complete AIH worktree slot: worktree, `.env`, copy-on-write `node_modules`, VS Code workspace entry, tmux session with `claude`/`rails`/`sidekiq`/`vite`/`shell` windows, and a Rails server |
